...
String token="eyJhbGciOiJIUzUxMiJ9..hkk7bbXUP-wkSjq615sSoyaYisS_GgProvzzeM9LZEfvoWVZBVpB-_Fxhu7DBF9-lQh5cRz0MH_VMcxwzmvJzA";

	RequestSpecification req = new RequestSpecBuilder()
			.setBaseUri("https://scrum4-api.fundsindia.com/core")
			//  .setBaseUri("https://staging-api.fundsindia.com/core")			//staging
			.addHeader("x-api-version", "1.0.0")
			.addHeader("channel-id", "10")
			.addHeader("x-fi-access-token", FD_Login.sathish())
		//	.addHeader("x-fi-access-token", token)			//staging
...
// 
	...
	public void Transaction() {
		...
		RequestSpecification res = given().spec(req)
				.body(trans);
		res.when().post("/investor/fixed-deposit/transactions")
				.then().log().all().spec(respec);
	}
...
//	search.put("query", "baja");
		...
		RequestSpecification res = given().spec(req)
				.body(search);
		res.when().post("/product-search/fixed-deposit/search-funds")
				.then().log().all().spec(respec);
	}
	...
	public void Retry() {
		RequestSpecification res = given().spec(req)
				.queryParam("fdId", "FD45404");
		res.when().get("/investor/fixed-deposit/investment/retry")
				.then().log().all().spec(respec);
	}
